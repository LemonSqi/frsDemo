...
public class FPR00102 
{ 	
	static String trxDaFldName = "trx_data";    // 交易数据字段
	static String devInfFldName = "dev_inf";    // 设备信息字段
	...
	static JSONObject devInfJson = new JSONObject(true);		// 设备信息子Json
	...
	public static void setReqJson() {
		...
		String trx_tp = "FPR00102";
		reqJson.put("trx_tp", trx_tp);                  	// 交易类型

		/* -----------------------------------添加交易数据到内层Json------------------------------------ */
		reqJson.put(trxDaFldName, trxDaJson);

		trxDaJson.put("upd_bitmap", "0010000000");		// 更新数据位图                                      

		String enc_key_sen = "ABCDEFABCDEFABCD1234567890123456"; 	// 敏感信息对称密钥（需要使用银联提供的加密公钥SM2算法进行加密）                                               

		// 路由索引（对6位人脸识别线下支付口令先SM3计算摘要，然后与“交易时间time_stamp”异或，再使用敏感信息对称密钥经SM4加密，最后base64编码）
		String pin = "000000";
		trxDaJson.put("index", HandleTrsDa.handelIndex(enc_key_sen, time_stamp, trx_tp, pin));
		...
		trxDaJson.put("name", "*Jones");              // 对原始姓名进行部分屏蔽

		/* ------------------------------------内层设备信息子Json------------------------------------ */
		reqJson.put(devInfFldName, devInfJson);

		devInfJson.put("dev_id", "23893HC394S34HZ");     // 设备ID（安卓：IMEI  IOS:IDFV）
		devInfJson.put("dev_tp", "1");                   // 设备类型（1：手机   2：平板）
		devInfJson.put("dev_md", "APPLE");				 // 设备型号  设备类型为“1-手机”时，本域出现
		devInfJson.put("ip_addr", "123.123.123.123");    // IP地址
		devInfJson.put("dev_lbs", "+37.12/-121.23");     // 经纬度
		...
		String encPubKey2 = "BAC6735F06888D4516484D5BFC575EE7F5E8B6DD7F5BDC0D172B2568148A2F2E";	  // 加密秘钥后半部分
